...
const Music = () => {
  ...
  useEffect(() => {
    ...
    /* idle-aware rAF: the loop only runs while the ring/glow are catching
       up to the cursor (or the tab is focused + mouse recently moved). It
       parks itself when everything has settled, and a mousemove restarts it.
       Saves battery vs. an always-on requestAnimationFrame loop. */
    let running = false;
    ...
    const frame = () => {
      ...
      // settled? (within sub-pixel of the target) → park the loop until next move
      if (Math.abs(mx - rx) < 0.4 && Math.abs(my - ry) < 0.4 && Math.abs(mx - gx) < 0.4 && Math.abs(my - gy) < 0.4) {
