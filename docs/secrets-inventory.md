...
n8n is **not** managed by this repository's docker-compose. It runs on the primary VPS (`srv1402218.hstgr.cloud`) as a separate Docker container or systemd service. It is provisioned separately (via the VPS setup scripts or manually). The public compose file (`public/empire/docker-compose.yml`) manages the observability stack and vault adapter only.

Two URL aliases exist for the same n8n instance:
- **Admin UI / API:** `https://n8n.srv1402218.hstgr.cloud` (used in scripts and setup docs)
- **Public webhooks:** `https://n8n.hansvanleeuwen.com` (behind Cloudflare; used in frontend config and workflow JSON)
...
| F-06 | `public/empire/docker-compose.yml` | 20 | **INFO** | `N8N_URL=https://n8n.hansvanleeuwen.com` hardcoded in compose. This is a non-secret URL, but it couples the compose file to a specific hostname. Consider using `${N8N_URL}` with a documented default. |
