...
const SortableToolCard = ({
  ...
  const catCfg = tool.category ? categoryConfig[tool.category] : null;
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`group relative flex h-full cursor-pointer flex-col rounded-xl border border-border bg-card text-left transition-all duration-300 hover:border-primary/30 hover:shadow-lg ${
          !isEditMode ? "hover:-translate-y-0.5" : ""
        } ${isDragging ? "shadow-2xl" : ""} p-3 sm:p-4`}
      ...
      >
        ...
        {isAdmin && !isEditMode && (
          <button
            ...
            className="absolute right-2 top-2 rounded-md p-1 text-muted-foreground/40 opacity-100 transition-all hover:bg-secondary hover:text-foreground md:opacity-0 md:group-hover:opacity-100 sm:right-2.5 sm:top-2.5"
        ...
        {/* ═══ MOBILE: Compact row — icon + name + category badge ═══ */}
        <div className={`flex items-center gap-2 sm:hidden ${isEditMode ? "mt-5" : ""}`}>
          ...
          {catCfg && (
            <span className={`shrink-0 rounded-full border px-1.5 py-px text-[8px] font-semibold uppercase tracking-wider ${catCfg.color}`}>
              {catCfg.label}
            </span>
          )}
        </div>

        {/* ═══ SM+: Full card header — icon + name/type + category ═══ */}
        <div className={`hidden items-center gap-2.5 sm:flex ${isEditMode ? "mt-5" : ""}`}>
          ...
          {catCfg && (
            <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${catCfg.color}`}>
              {catCfg.label}
            </span>
          )}
        </div>
